...
??? tip "Gợi ý"
    - Dùng `String.format()` hoặc `System.out.printf()` để căn chỉnh cột
...
??? success "Lời giải"
    ...
    **Điểm cần chú ý:**
    ...
    - `%-16s` — căn trái, padding đủ 16 ký tự để khung không bị lệch
    - `%.2f` — in 2 chữ số thập phân cho GPA
    - `var` được suy ra `String` và `double` từ initializer
    - `20240001L` cần hậu tố `L` vì kiểu `long`
