# Bài tập: Kiểu dữ liệu và Biến

Bài tập thực hành cho bài [Kiểu dữ liệu và Biến](../fundamentals/02-variables-datatypes.md).
...
2. Gán dữ liệu mẫu cho từng biến.
...
??? tip "Gợi ý"
    - Dùng string concatenation (`+`) hoặc `String.format()` để build từng dòng
    - `boolean` → `"Enrolled"` / `"Not Enrolled"` có thể dùng toán tử ternary: `isEnrolled ? "Enrolled" : "Not Enrolled"`
    - Các ký tự `╔`, `╠`, `╚`, `║` có thể copy từ đây hoặc escape: `╔`, `╠`, `╚`, `║`
...
??? success "Lời giải"
    ...
    **Điểm cần chú ý:**
    ...
    - `%-16s` — căn trái, đủ 16 ký tự để giữ đúng độ rộng khung
    - `%.2f` — in 2 chữ số thập phân cho GPA
    - `var` được dùng cho `name` và `gpa` — compiler suy ra `String` và `double`
    - `20240001L` cần hậu tố `L` vì `long`
