...
Practice exercises for [Variables & Data Types](../fundamentals/02-variables-datatypes.md).
...
??? success "Solution"
    ...
    | (C) | `19.99` is a `double` literal — assigning it to `float` loses precision, compiler rejects it | Add suffix `f`: `19.99f`, or change type to `double` |
...
??? tip "Hint"
    - Use string concatenation (`+`) or `String.format()` to build each line
    - For `boolean` → `"Enrolled"` / `"Not Enrolled"`, use the ternary operator: `isEnrolled ? "Enrolled" : "Not Enrolled"`
    - Border characters `╔`, `╠`, `╚`, `║` can be copy-pasted from here
...
??? success "Solution"
    ...
    **Key points:**
    ...
    - `%-16s` — left-align, pad to 16 characters to maintain border alignment
    - `%.2f` — print GPA to 2 decimal places
    - `var` used for `name` and `gpa` — compiler infers `String` and `double`
    - `20240001L` needs the `L` suffix since the type is `long`
