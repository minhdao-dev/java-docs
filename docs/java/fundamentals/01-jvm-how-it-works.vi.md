...
!!! note "Write once, run anywhere"
    File `.class` bytecode hoàn toàn giống nhau trên Windows, macOS và Linux.
    JVM trên mỗi OS là khác nhau — nhưng code thì không.
...
!!! info "Java 25 — Compact Object Headers (JEP 519)"
    ...
    Trước Java 25, header này chiếm **12–16 bytes** mỗi object (12 bytes khi compressed oops bật — mặc định của JVM; 16 bytes khi tắt).
...
**Native Method Stack** hỗ trợ các method C/C++ được gọi qua JNI — hiếm khi cần tương tác trực tiếp với cái này.
...
!!! note "Hay bị nhầm — lịch sử GC"
    ...
    **Java 25 LTS** là baseline hiện tại. G1GC vẫn là mặc định. Java 21 LTS vẫn được hỗ trợ song song và là LTS phổ biến nhất trong production hiện tại (2026).
...
!!! info "Verified"
    Bản đầy đủ có thể compile: [`JvmMemoryDemo.java`](https://github.com/minhdao-dev/java-docs/blob/main/examples/src/main/java/fundamentals/jvm/JvmMemoryDemo.java)
